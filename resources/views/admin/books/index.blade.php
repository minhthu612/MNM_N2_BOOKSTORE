    ...
    <div class="card the-bang border-0 shadow-sm">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.books.index') }}" class="row g-2 mb-4">
                <div class="col-md-4">
                    <input type="text" class="form-control px-3" name="search" value="{{$search}}" placeholder="Tìm kiếm tên sách...">
                </div>
                            ...
                            <tr class="text-center">
                                ...
                                <td>
                                    @php
                                        ...
                                        $img = 'https://via.placeholder.com/60x80?text=No+Image';

                                        foreach ($extensions as $ext) {
                                            if (file_exists(storage_path("app/public/image/{$b->book_id}.{$ext}"))) {
                                                $img = asset("storage/image/{$b->book_id}.{$ext}");
                                                break;
                                            }
                                        }

                                        // Nếu không có trong storage mới dùng link_images
                                        if ($img == 'https://via.placeholder.com/60x80?text=No+Image' && !empty($b->link_images)) {
                                            $img = $b->link_images;
                                        }
                                    @endphp
                                    <img src="{{$img}}" class="anh-sach-nho" onerror="this.src='https://via.placeholder.com/60x80?text=No+Image'">
                                </td>
