...
<div class="container mt-4">
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 mb-4">
            <div class="bg-white p-3 rounded shadow-sm h-100 book-card">
                ...
                <a href="{{ route('wishlist.add', ['book_id' => $book->book_id]) }}" class="btn-wishlist">
                    <i class="{{ in_array($book->book_id, $wishlist_ids ?? []) ? 'fas text-danger' : 'far' }} fa-heart"></i>
                </a>

                <div class="text-center position-relative">
                    <img src="{{ asset($book->link_images ?? 'images/no-image.jpg') }}" class="img-fluid mb-2" style="height:180px; object-fit:contain;">
                </div>
                ...
                <div class="mt-3 d-flex justify-content-between align-items-center">
                    <div class="text-muted small"><i class="fas fa-shopping-cart"></i> {{ $book->sold_quantity ?? 0 }}</div>
                    <a href="{{ url((isset($book->loai) && $book->loai == 'set' ? '/book-set/' : '/books/').$book->book_id) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">Chi tiết</a>
                </div>
            </div>
        </div>
    @empty
    ...
    <div class="row mt-4 mb-5">
        <div class="col-12"><nav><ul class="pagination justify-content-center">{{ $books->links('pagination::bootstrap-5') }}</ul></nav></div>
    </div>
    @endif
</div>
@endsection