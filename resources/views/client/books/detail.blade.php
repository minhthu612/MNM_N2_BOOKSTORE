...
/* Nút trái tim kiểu mới - Đã hỗ trợ trạng thái Active */
.nut-yeu-thich-tron {
    width: 55px;
    height: 55px;
...
<div class="container py-5">
    ...
    <div class="khung-chi-tiet shadow-sm">
        <div class="row g-5">
            ...
            <div class="col-md-5">
                <div class="text-center bg-light p-4 rounded-4 d-flex align-items-center justify-content-center" style="min-height: 450px;">
                    <img src="{{ asset($book->link_images ?? 'images/no-image.jpg') }}" class="anh-sach-lon">
                </div>
            </div>
            ...
            <div class="col-md-7">
                ...
                <div class="mb-3">
                    ...
                    @if(($book->stock ?? 0) > 0)
                        <span class="badge rounded-pill bg-success px-3 py-2">Còn hàng ({{ $book->stock }})</span>
                    @else
                        <span class="badge rounded-pill bg-danger px-3 py-2">Hết hàng</span>
                    @endif
                </div>

                <h1 class="fw-bold mb-3" style="font-size: 3rem;">{{ $book->title }}</h1>
                ...
                <div class="mb-4">
                    <div class="d-flex align-items-center">
                        ...
                        @if($giam > 0)
                            ...
                            <span class="badge badge-disc-large ms-3">-{{ $giam }}%</span>
                        @endif
                    </div>
                </div>
                ...
                <form action="{{ route('cart.add') }}" method="POST">
                    ...
                    <div class="d-flex align-items-center gap-4 mb-4">
                        ...
                        <input type="number" name="quantity" value="1" min="1" max="{{ $book->stock }}" class="form-control o-nhap-sl fs-5 p-2">
                        <span class="text-muted">Có sẵn: {{ $book->stock }}</span>
                    </div>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <a href="{{ route('wishlist.add', ['book_id' => $book->book_id]) }}"
                           class="btn nut-yeu-thich-tron shadow-sm {{ $da_thich ? 'active' : '' }}">
                            {{-- Nếu đã thích hiện fas (đậm), chưa thích hiện far (rỗng) --}}
        ...
        <div class="row mt-5 pt-5 border-top">
            <div class="col-md-8">
                <h5 class="fw-bold text-primary mb-4 text-uppercase" style="letter-spacing: 1px;">Giới thiệu sách</h5>
                <div class="text-muted fs-5" style="line-height:1.9">
                    {!! $book->description ? nl2br($book->description) : 'Đang cập nhật nội dung...' !!}
                </div>

                <h5 class="fw-bold text-primary mt-5 mb-4 text-uppercase" style="letter-spacing: 1px;">Đánh giá từ bạn đọc</h5>
                @forelse($list_reviews as $rev)
                    <div class="bg-light p-4 rounded-4 mb-3 border-0">
                        ...
                        <p class="mb-0 text-secondary italic">"{!! nl2br($rev->comment) !!}"</p>
                    </div>
                @empty
            ...
            <div class="col-md-4">
                <div class="bg-light p-4 rounded-4 sticky-top" style="top: 100px;">
                    ...
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2 small d-flex align-items-center"><i class="fas fa-check-circle text-success me-2"></i> Giao hàng toàn quốc</li>
                        <li class="mb-2 small d-flex align-items-center"><i class="fas fa-check-circle text-success me-2"></i> Đổi trả 7 ngày</li>
                        <li class="mb-2 small d-flex align-items-center"><i class="fas fa-check-circle text-success me-2"></i> Sách chính bản 100%</li>
                        <li class="small d-flex align-items-center"><i class="fas fa-check-circle text-success me-2"></i> Kiểm hàng trước khi nhận</li>
                    </ul>
                </div>
            </div>
        </div>
