...
class CartController extends Controller
{
   ...
   // ================= ADD (SÁCH LẺ) - ĐÃ CHECK OK =================
    public function add(Request $request)
    {
        $user_id = auth()->id();
        if (!$user_id) {
            // Trả về 401 để JavaScript biết và chuyển hướng sang /login
        ...
        if ($book_id <= 0) return response()->json(['status' => 'error', 'message' => 'Sản phẩm không hợp lệ'], 400);
        ...
        // Tùy chọn: Xóa coupon khi giỏ hàng thay đổi để tránh sai lệch số tiền nếu bạn muốn
        // session()->forget(['discount_amount', 'applied_coupon']);


        // Lấy tổng số loại sản phẩm trong giỏ để cập nhật Badge
        $totalCount = DB::table('cart_items')->where('cart_id', $cart_id)->count();
