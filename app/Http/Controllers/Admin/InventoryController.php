...
class InventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.inventory.index', compact(
            'inventory_list', 'stats', 'search', 'status', 'stock_filter', 'page', 'total_pages', 'total_items'
        ));
    }

    // ================= HISTORY (Bây giờ chỉ hiển thị danh sách kho hiện tại) =================
    public function history(Request $request)
    {
        ...
        return view('admin.inventory.history', compact(
            'inventory_list', 'stats', 'search', 'status_filter', 'page', 'total_pages', 'total_items'
        ));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Xử lý logic số lượng và trạng thái
        if ($new_stock <= 0) {
        ...
        /* PHẦN GHI LOG ĐÃ BỊ XÓA BỎ ĐỂ TRÁNH LỖI "Table inventory_history doesn't exist"
        */

        return redirect()->route('admin.inventory.history')
            ->with('success', 'Đã cập nhật số lượng tồn kho thành công!');
    }
}